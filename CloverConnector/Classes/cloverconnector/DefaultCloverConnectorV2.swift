...
class DefaultCloverConnectorV2 : NSObject, ICloverConnector {
    ...
    @objc
    public func retrievePrinters(request: RetrievePrintersRequest) {
        checkDevice(from: #function)?.doRetrievePrinters(request)
    }
    
    @objc
    public func retrievePrintJobStatus(request: PrintJobStatusRequest) {
    ...
    @objc(openCashDrawerWithRequest:)
    ...
    private func checkDevice(from funcName: String) -> CloverDevice? {
    ...
    func sanitizedFunctionName(funcName: String) -> String {
        /*
        ...
        if let regex = try? NSRegularExpression(pattern: "\\(([^\\)]+)\\)", options: .CaseInsensitive) {
            let sanitizedFunctionName = regex.stringByReplacingMatchesInString(funcName, options: .WithTransparentBounds, range: NSMakeRange(0, funcName.characters.count), withTemplate: "")
    ...
    class CloverConnectorDeviceObserver : CloverDeviceObserver {
        ...
        func onDeviceError(errorType: CloverDeviceErrorType, int: Int?, cause:NSError?, message: String) {
        ...
        func onFinishOk(_ refund: CLVModels.Payments.Refund, requestInfo:String?) {
            ...
            if self.lastPRR != nil && self.lastPRR?.refund?.id == refund.id {
                cloverConnector.broadcaster.notifyOnPaymentRefundResponse(self.lastPRR!);
                self.lastPRR = nil
            } else {
                let rpr = RefundPaymentResponse(success:true, result: ResultCode.SUCCESS, orderId: refund.orderRef?.id, paymentId:refund.payment?.id, refund:refund)
